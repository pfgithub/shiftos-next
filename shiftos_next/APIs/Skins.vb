...
#Region "Basic Window Manager"
    ...
    Public Sub loadskin_bwm(file As String)
...
#End Region
    ...
    Public Sub loadsknfile(ByVal filepath As String)
        If IO.Directory.Exists(sde_skindir) Then IO.Directory.Delete(sde_skindir)
        IO.Directory.CreateDirectory(sde_skindir)
        System.IO.Compression.ZipFile.ExtractToDirectory(filepath, sde_skindir)
        loadimages()
    End Sub
    ' LOAD SKIN FROM SAVE FOLDER
    Public Sub loadimages()
        If File.Exists(sde_skindir & "userpic") Then
            userimage = GetImage(sde_skindir & "userpic")
        End If
        If File.Exists(sde_skindir & "loginbg") Then
            loginbg = GetImage(sde_skindir & "loginbg")
        End If
        If File.Exists(sde_skindir & "userbar") Then
            userNamePanelBackground = GetImage(sde_skindir & "userbar")
        End If
        If File.Exists(sde_skindir & "powerbar") Then
            powerPanelBackgroundImage = GetImage(sde_skindir & "powerbar")
        End If
        If File.Exists(sde_skindir & "titlebar") Then
            titlebar = GetImage(sde_skindir & "titlebar")
        Else : titlebar = Nothing
        End If
        If File.Exists(sde_skindir & "borderleft") Then
            borderleft = GetImage(sde_skindir & "borderleft")
        Else : borderleft = Nothing
        End If
        If File.Exists(sde_skindir & "borderright") Then
            borderright = GetImage(sde_skindir & "borderright".Clone)
        Else : borderright = Nothing
        End If
        If File.Exists(sde_skindir & "borderbottom") Then
            borderbottom = GetImage(sde_skindir & "borderbottom".Clone)
        Else : borderbottom = Nothing
        End If
        If File.Exists(sde_skindir & "closebtn") Then
            closebtn = GetImage(sde_skindir & "closebtn".Clone)
        Else : closebtn = Nothing
        End If
        If File.Exists(sde_skindir & "closebtnhover") Then
            closebtnhover = GetImage(sde_skindir & "closebtnhover".Clone)
        Else : closebtnhover = Nothing
        End If
        If File.Exists(sde_skindir & "closebtnclick") Then
            closebtnclick = GetImage(sde_skindir & "closebtnclick".Clone)
        Else : closebtnclick = Nothing
        End If
        If File.Exists(sde_skindir & "rollbtn") Then
            rollbtn = GetImage(sde_skindir & "rollbtn".Clone)
        Else : rollbtn = Nothing
        End If
        If File.Exists(sde_skindir & "rollbtnhover") Then
            rollbtnhover = GetImage(sde_skindir & "rollbtnhover".Clone)
        Else : rollbtnhover = Nothing
        End If
        If File.Exists(sde_skindir & "rollbtnclick") Then
            rollbtnclick = GetImage(sde_skindir & "rollbtnclick".Clone)
        Else : rollbtnclick = Nothing
        End If
        If File.Exists(sde_skindir & "minbtn") Then
            minbtn = GetImage(sde_skindir & "minbtn".Clone)
        Else : minbtn = Nothing
        End If
        If File.Exists(sde_skindir & "minbtnhover") Then
            minbtnhover = GetImage(sde_skindir & "minbtnhover".Clone)
        Else : minbtnhover = Nothing
        End If
        If File.Exists(sde_skindir & "minbtnclick") Then
            minbtnclick = GetImage(sde_skindir & "minbtnclick".Clone)
        Else : minbtnclick = Nothing
        End If
        If File.Exists(sde_skindir & "rightcorner") Then
            rightcorner = GetImage(sde_skindir & "rightcorner".Clone)
        Else : rightcorner = Nothing
        End If
        If File.Exists(sde_skindir & "leftcorner") Then
            leftcorner = GetImage(sde_skindir & "leftcorner".Clone)
        Else : leftcorner = Nothing
        End If
        If File.Exists(sde_skindir & "desktoppanel") Then
            desktoppanel = GetImage(sde_skindir & "desktoppanel".Clone)
        Else : desktoppanel = Nothing
        End If
        If File.Exists(sde_skindir & "desktopbackground") Then
            desktopbackground = GetImage(sde_skindir & "desktopbackground".Clone)
        Else : desktopbackground = Nothing
        End If
        If File.Exists(sde_skindir & "panelbutton") Then
            panelbutton = GetImage(sde_skindir & "panelbutton".Clone)
        Else : panelbutton = Nothing
        End If
        If File.Exists(sde_skindir & "applaunchermouseover") Then
            applaunchermouseover = GetImage(sde_skindir & "applaunchermouseover".Clone)
        Else : applaunchermouseover = Nothing
        End If
        If File.Exists(sde_skindir & "applauncher") Then
            applauncher = GetImage(sde_skindir & "applauncher".Clone)
        Else : applauncher = Nothing
        End If
        If File.Exists(sde_skindir & "applauncherclick") Then
            applauncherclick = GetImage(sde_skindir & "applauncherclick".Clone)
        Else : applauncherclick = Nothing
        End If
        If File.Exists(sde_skindir & "panelclock") Then
            panelclock = GetImage(sde_skindir & "panelclock".Clone)
        Else : panelclock = Nothing
        End If
        If File.Exists(sde_skindir & "bottomleftcorner") Then
            bottomleftcorner = GetImage(sde_skindir & "bottomleftcorner".Clone)
        Else : bottomleftcorner = Nothing
        End If
        If File.Exists(sde_skindir & "bottomrightcorner") Then
            bottomrightcorner = GetImage(sde_skindir & "bottomrightcorner".Clone)
        Else : bottomrightcorner = Nothing
        End If
        'load settings
        Dim loaddata(200) As String
        If File.Exists(sde_skindir & "data.dat") Then
            Dim sr As StreamReader = New StreamReader(sde_skindir & "data.dat")
    ...
    Public Sub saveskin(ByVal path As String)
        If Not File.Exists(path) Then
            Dim sw As StreamWriter = New StreamWriter(sde_skindir & "SKN-version") 'tells skin loader which system to use when openning the file
            sw.WriteLine("Name of skinning system used to create this skn file:")
            sw.WriteLine("NEXT - Orion Engine: Multiple Desktop Environment Skinning System")
            sw.WriteLine("Skinning system created by Michael VanOverbeek, with parts of code by william.1008 and Philip Adams.")
            sw.WriteLine(" ")
            sw.WriteLine("Desktop Environment used to create skin: SHIFTER'S_DESKTOP")
            sw.Close()
            saveskinfiles(False)
            Compression.ZipFile.CreateFromDirectory(sde_skindir, path)
            File.Delete(sde_skindir & "SKN-version")
        Else
    ...
    Public Sub saveskinfiles(ByVal apply As Boolean)
        If File.Exists(sde_skindir) Then
            Directory.Delete(sde_skindir)
        End If
        Directory.CreateDirectory(sde_skindir)
        ...
        File.WriteAllLines(sde_skindir & "data.dat", savedata)
        If apply = True Then
            applyskin()
        End If
    End Sub
    ...
    Private Sub saveimage(ByVal img As Image, ByVal name As String)
        If Not IsNothing(img) Then
            If File.Exists(sde_skindir & name) Then File.Delete(sde_skindir & name)
            Try
                img.Save(sde_skindir & name, System.Drawing.Imaging.ImageFormat.Png)
            Catch ex As Exception
                infobox.showinfo("Saving Error", "Oops, an error occured when saving the file " & sde_skindir & name & ". Please contact the devs to report the problem.")
            End Try
        Else
            If File.Exists(sde_skindir & name) Then File.Delete(sde_skindir & name)
        End If
